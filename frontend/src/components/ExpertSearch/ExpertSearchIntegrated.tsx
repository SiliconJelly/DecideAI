...
import { useExpertSearch } from '../../hooks/useApi';
import { transformBackendExpertToFrontend, createMockAvailabilityStatus, createMockRating } from '../../utils/dataTransform';
...
interface ExpertSearchIntegratedProps {
  onExpertSelect?: (expert: ExpertProfile) => void;
}

export default function ExpertSearchIntegrated({ onExpertSelect }: ExpertSearchIntegratedProps) {
  ...
  // Use the API hook for real backend data
  const { data: backendExperts, error, loading } = useExpertSearch(searchQuery, filters);
  
  // Transform backend data to frontend format
  const [transformedExperts, setTransformedExperts] = useState<ExpertProfile[]>([]);

  useEffect(() => {
    if (backendExperts) {
      const transformed = backendExperts.map(expert => ({
        ...transformBackendExpertToFrontend(expert),
        // Add mock data for fields not yet in backend
        availability: createMockAvailabilityStatus(),
        rating: createMockRating()
      }));
      setTransformedExperts(transformed);
    }
  }, [backendExperts]);
  ...
  const filteredExperts = transformedExperts.filter(expert => {
    ...
    return matchesAvailability && matchesExperience && matchesLanguages;
  }).sort((a, b) => {
    ...
    return (
      <div className={styles.expertSearch}>
        <div className={styles.header}>
          <h1>Find Experts</h1>
          <p>Loading experts from backend...</p>
        </div>
        <div className={styles.loadingContainer}>
          <LoadingSpinner size="lg" />
          <p>Fetching expert data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className={styles.resultsHeader}>
        <div className={styles.resultsInfo}>
          ...
          <Badge variant="info" size="sm">
            Backend: {transformedExperts.length} total
          </Badge>
        </div>
